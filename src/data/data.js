...
export const DOM = [
  ...
  },
  {
    id: 4,
    category: 4,
    ComponentName: "MouseEvents",
    name: "События мыши в DOM",
    path: "/dom/mouse-event",
    defaultPath: "mouse-event",
    data: [
      {
        title: "click - клик по элементу",
        desc: "Событие `click` срабатывает при щелчке мышью по элементу. Вызывается после `mousedown` и `mouseup`.",
      },
      {
        title: "dblclick - двойной клик",
        desc: "Событие `dblclick` срабатывает при быстром двойном клике по элементу.",
      },
      {
        title: "mousedown - нажатие кнопки мыши",
        desc: "Событие `mousedown` срабатывает, когда пользователь нажимает кнопку мыши на элементе.",
      },
      {
        title: "mouseup - отпускание кнопки мыши",
        desc: "Событие `mouseup` срабатывает, когда пользователь отпускает кнопку мыши после `mousedown`.",
      },
      {
        title: "mousemove - перемещение мыши",
        desc: "Событие `mousemove` срабатывает при каждом движении курсора мыши над элементом.",
      },
      {
        title: "mouseenter - вход курсора в границы элемента",
        desc: "Событие `mouseenter` срабатывает, когда курсор заходит в границы элемента. Не всплывает.",
      },
      {
        title: "mouseleave - выход курсора за границы элемента",
        desc: "Событие `mouseleave` срабатывает, когда курсор покидает границы элемента. Не всплывает.",
      },
      {
        title: "mouseover - наведение курсора",
        desc: "Событие `mouseover` срабатывает, когда курсор попадает в границы элемента. Всплывает.",
      },
      {
        title: "mouseout - покидание элемента",
        desc: "Событие `mouseout` срабатывает, когда курсор выходит за границы элемента. Всплывает.",
      },
      {
        title: "contextmenu - клик правой кнопкой мыши",
        desc: "Событие `contextmenu` срабатывает при клике правой кнопкой мыши, вызывая контекстное меню браузера.",
      },
      {
        title: "wheel - прокрутка колесика мыши",
        desc: "Событие `wheel` срабатывает при прокручивании колесика мыши и предоставляет информацию о величине прокрутки.",
      },
      {
        title: "scroll - прокрутка страницы",
        desc: "Событие `scroll` срабатывает, когда пользователь прокручивает страницу или элемент с прокруткой.",
      },
      {
        title: "pointerdown - нажатие кнопки указателя",
        desc: "Событие `pointerdown` срабатывает, когда пользователь нажимает кнопку мыши, стилус или экранный тач.",
      },
      {
        title: "pointerup - отпускание кнопки указателя",
        desc: "Событие `pointerup` срабатывает, когда пользователь отпускает кнопку мыши, стилус или палец.",
      },
      {
        title: "pointermove - перемещение указателя",
        desc: "Событие `pointermove` срабатывает при каждом движении указателя (мышь, стилус, палец).",
      },
      {
        title: "pointerenter - вход указателя в элемент",
        desc: "Событие `pointerenter` срабатывает при заходе указателя в границы элемента. Не всплывает.",
      },
      {
        title: "pointerleave - выход указателя из элемента",
        desc: "Событие `pointerleave` срабатывает при выходе указателя за границы элемента. Не всплывает.",
      },
      {
        title: "pointerover - наведение указателя",
        desc: "Событие `pointerover` срабатывает, когда указатель попадает в границы элемента. Всплывает.",
      },
      {
        title: "pointerout - покидание элемента указателем",
        desc: "Событие `pointerout` срабатывает, когда указатель выходит за границы элемента. Всплывает.",
      },
      {
        title: "dragstart - начало перетаскивания",
        desc: "Событие `dragstart` срабатывает, когда пользователь начинает перетаскивание элемента.",
      },
      {
        title: "drag - перетаскивание",
        desc: "Событие `drag` срабатывает при каждом перемещении элемента в процессе перетаскивания.",
      },
      {
        title: "dragend - завершение перетаскивания",
        desc: "Событие `dragend` срабатывает, когда пользователь завершает перетаскивание элемента.",
      },
      {
        title: "dragenter - вход в зону перетаскивания",
        desc: "Событие `dragenter` срабатывает, когда элемент, который перетаскивают, заходит в зону приема.",
      },
      {
        title: "dragover - нахождение над зоной перетаскивания",
        desc: "Событие `dragover` срабатывает, когда элемент находится в зоне приема и перемещается внутри нее.",
      },
      {
        title: "dragleave - выход из зоны перетаскивания",
        desc: "Событие `dragleave` срабатывает, когда перетаскиваемый элемент покидает зону приема.",
      },
      {
        title: "drop - отпускание элемента в зоне перетаскивания",
        desc: "Событие `drop` срабатывает, когда перетаскиваемый элемент отпускается в зоне приема.",
      },
      {
        title: "selectstart - начало выделения",
        desc: "Событие `selectstart` срабатывает, когда пользователь начинает выделять текст или элементы.",
      },
      {
        title: "select - выделение текста",
        desc: "Событие `select` срабатывает, когда пользователь завершает выделение текста.",
      },
      {
        title: "mouseleave + setTimeout - задержка скрытия",
        desc: "Комбинирование `mouseleave` с `setTimeout` используется для задержки скрытия всплывающих элементов.",
      },
      {
        title: "mousemove + requestAnimation Frame - сглаживание анимации",
        desc: "Использование `mousemove` в связке с `requestAnimationFrame` позволяет добиться плавных анимаций курсора.",
      },
    ],
  },
  {
    id: 5,
    category: 5,
    ComponentName: "MouseEventProps",
    name: "Свойства события",
    path: "/dom/mouse-event-props",
    defaultPath: "mouse-event-props",
    data: [
      {
        title: "Всплытие событий (Event Bubbling)",
        desc: "Всплытие (bubbling) - это процесс, при котором событие сначала обрабатывается на целевом элементе, а затем поднимается вверх по DOM-дереву. Можно остановить всплытие с помощью `event.stopPropagation()`.",
      },
      {
        title: "Погружение событий (Event Capturing)",
        desc: "Погружение (capturing) - это процесс, при котором событие сначала идет сверху вниз, прежде чем достичь целевого элемента. Чтобы включить погружение, передайте `true` третьим аргументом в `addEventListener()`.",
      },
      {
        title: "e.target - целевой элемент",
        desc: "Свойство `e.target` указывает на элемент, на котором произошло событие. Всплывающие события могут иметь `target`, отличающийся от текущего элемента обработчика.",
      },
      {
        title: "e.currentTarget - текущий элемент обработчика",
        desc: "Свойство `e.currentTarget` указывает на элемент, на котором выполняется обработчик события. В отличие от `e.target`, оно не меняется при всплытии.",
      },
      {
        title: "e.preventDefault() - предотвращение стандартного поведения",
        desc: "Метод `e.preventDefault()` предотвращает стандартное действие элемента. Например, запрет отправки формы или контекстного меню.",
      },
      {
        title: "e.stopPropagation() - остановка всплытия",
        desc: "Метод `e.stopPropagation()` предотвращает дальнейшее всплытие события вверх по DOM-дереву.",
      },
      {
        title: "e.stopImmediate Propagation() - остановка всех обработчиков",
        desc: "Метод `e.stopImmediatePropagation()` предотвращает выполнение всех остальных обработчиков на текущем элементе.",
      },
      {
        title: "e.relatedTarget - связанный элемент",
        desc: "Свойство `e.relatedTarget` используется в событиях `mouseenter`, `mouseleave`, `mouseover`, `mouseout` и указывает элемент, с которого или на который ушел курсор.",
      },
      {
        title: "offsetX / offsetY - координаты относительно родителя",
        desc: "Свойства `offsetX` и `offsetY` указывают координаты курсора относительно границ целевого элемента.",
      },
      {
        title: "clientX / clientY - координаты относительно окна",
        desc: "Свойства `clientX` и `clientY` показывают координаты курсора относительно окна браузера, без учета прокрутки.",
      },
      {
        title: "pageX / pageY - координаты относительно документа",
        desc: "Свойства `pageX` и `pageY` показывают координаты курсора относительно всей страницы, включая прокрутку.",
      },
      {
        title: "screenX / screenY - координаты относительно экрана",
        desc: "Свойства `screenX` и `screenY` указывают координаты курсора относительно экрана пользователя.",
      },
      {
        title: "e.buttons - состояние кнопок мыши",
        desc: "Свойство `e.buttons` возвращает битовую маску нажатых кнопок мыши. Например, 1 - левая, 2 - правая, 4 - средняя.",
      },
      {
        title: "e.button - код нажатой кнопки",
        desc: "Свойство `e.button` указывает, какая кнопка мыши была нажата: 0 - левая, 1 - средняя, 2 - правая.",
      },
      {
        title: "wheelDelta - прокрутка колесом мыши",
        desc: "Свойство `wheelDelta` (устарело) показывает, на сколько изменилось положение колеса мыши. Для современных браузеров используйте `deltaY`.",
      },
      {
        title: "deltaX / deltaY / deltaZ - изменения прокрутки",
        desc: "Свойства `deltaX`, `deltaY`, `deltaZ` показывают, на сколько прокручен контент по соответствующим осям.",
      },
      {
        title: "e.metaKey - нажатие Meta (Command на Mac)",
        desc: "Свойство `e.metaKey` возвращает `true`, если во время события была зажата клавиша Meta (Command на Mac).",
      },
      {
        title: "e.shiftKey - нажатие Shift",
        desc: "Свойство `e.shiftKey` возвращает `true`, если во время события была зажата клавиша Shift.",
      },
      {
        title: "e.ctrlKey - нажатие Ctrl",
        desc: "Свойство `e.ctrlKey` возвращает `true`, если во время события была зажата клавиша Ctrl.",
      },
      {
        title: "e.altKey - нажатие Alt",
        desc: "Свойство `e.altKey` возвращает `true`, если во время события была зажата клавиша Alt.",
      },
      {
        title: "dblclick - двойной клик",
        desc: "Событие `dblclick` вызывается при двойном клике на элемент.",
      },
      {
        title: "contextmenu - контекстное меню",
        desc: "Событие `contextmenu` вызывается при клике правой кнопкой мыши. Можно отменить стандартное поведение с `e.preventDefault()`.",
      },
      {
        title: "mouseenter / mouseleave - наведение без всплытия",
        desc: "События `mouseenter` и `mouseleave` срабатывают при входе/выходе курсора на элемент и не всплывают.",
      },
      {
        title: "mouseover / mouseout - наведение с всплытием",
        desc: "События `mouseover` и `mouseout` срабатывают при наведении/выходе курсора, но всплывают и могут реагировать на дочерние элементы.",
      },
      {
        title: "mousemove - движение мыши",
        desc: "Событие `mousemove` вызывается при каждом движении мыши. Используйте `requestAnimationFrame()` для оптимизации производительности.",
      },
      {
        title: "mousedown - нажатие кнопки мыши",
        desc: "Событие `mousedown` срабатывает при нажатии кнопки мыши.",
      },
      {
        title: "mouseup - отпускание кнопки мыши",
        desc: "Событие `mouseup` срабатывает при отпускании кнопки мыши.",
      },
      {
        title: "drag - перетаскивание элемента",
        desc: "Событие `drag` срабатывает при начале перетаскивания элемента.",
      },
      {
        title: "dragstart - начало перетаскивания",
        desc: "Событие `dragstart` срабатывает, когда пользователь начинает перетаскивание элемента.",
      ...
      {
        title: "drop - сброс перетаскиваемого элемента",
        desc: "Событие `drop` срабатывает, когда перетаскиваемый элемент сбрасывается на приемник.",
      },
      {
        title: "dragover - перемещение элемента над зоной приема",
        desc: "Событие `dragover` срабатывает, когда элемент перетаскивается над допустимой зоной сброса.",
      },
      {
        title: "dragenter - вход элемента в зону приема",
        desc: "Событие `dragenter` срабатывает, когда элемент входит в допустимую зону сброса.",
      },
      {
        title: "dragleave - выход элемента из зоны приема",
        desc: "Событие `dragleave` срабатывает, когда элемент покидает допустимую зону сброса.",
      },
    ],
  },
  {
...
export const Themes = [
  ...
  },
  {
    ...
    name: "CSS (Cascading Style Sheets, каскадные таблицы стилей)",
    title:
      " — это язык стилей, который используется для описания внешнего вида и форматирования HTML-документов.",
  },
  {
    id: 3,
    them: "js",
    ...
    name: "JavaScript (JS)",
    title:
      " — это язык программирования, который используется для создания интерактивных и динамичных веб-страниц.",
  },
  {
    id: 4,
    them: "dom",
    href: "DOM (Document Object Model)",
    name: "HTML (HyperText Markup Language)",
    title:
      "  — это программный интерфейс для веб-документов. Он представляет собой структурированное представление документа, которое позволяет скриптам изменять содержимое",
  },
  {
    id: 5,
    them: "react",
    ...
    name: "React",
    title:
      " — это популярная библиотека JavaScript для создания пользовательских интерфейсов. Она позволяет разрабатывать веб-приложения, которые динамически обновляют и рендерят интерфейс без необходимости перезагрузки страницы.",
  },
];
